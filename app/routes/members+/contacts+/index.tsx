...
} from '#app/routes/members+/contacts+/contacts.server'
...
export default function ContactsRoute() {
	const { contacts, tableParams, displays, total } =
					...
					({
						...
						<div key={userId} className="grid grid-cols-12 gap-1">
								...
								<Input
									...
									onBlur={(e) =>
										handleChange({
											userId,
											intent: 'quickbooks',
											quickbooks: e.currentTarget.value,
										})
									}
								/>
							</div>
							<div className="col-span-3 flex flex-col">
										...
										<Input
											...
											onBlur={(e) =>
												handleChange({
													...
													intent: 'phone-type',
													phoneId,
													phoneType: e.currentTarget.value,
												})
											}
										/>
								...
								<CreatePhone
									key={`create-${userId}-${phones.length}`}
									userId={userId}
									index={phones.length}
								/>
							</div>
									...
									<Input
										...
										onBlur={(e) =>
											handleChange({
												userId,
												intent: 'emailSubject',
												emailSubject: e.currentTarget.value,
											})
										}
									/>
									<Input
										...
										onBlur={(e) =>
											handleChange({
												userId,
												intent: 'primaryEmail',
												primaryEmail: e.currentTarget.value,
											})
										}
									/>
								</div>
								<div className="grid grid-cols-4">
									...
									<Input
										...
										onBlur={(e) =>
											handleChange({
												userId,
												intent: 'secondarySubject',
												secondarySubject: e.currentTarget.value,
											})
										}
									/>
									<Input
										...
										onBlur={(e) =>
											handleChange({
												userId,
												intent: 'secondaryEmail',
												secondaryEmail: e.currentTarget.value,
											})
										}
									/>
								</div>
...
function CreatePhone({ userId, index }: { userId: string; index: number }) {
	...
	const [form, fields] = useForm({
		id: `${createPhoneActionIntent}-${userId}`,
	...
	return (
		...
		>
			...
			<input type="hidden" name="intent" value={createPhoneActionIntent} />
			<Label
				className="col-span-1 m-1"
				htmlFor="emailSubject"
				children="Type"
			...
			<Input
				className="col-span-1 rounded-r-none capitalize"
				{...getInputProps(fields.phoneType, { type: 'text' })}
			/>
...
function DeletePhone({ phoneId }: { phoneId: string }) {
	...
	const isPending = useIsPending()
	return (
		<Form method="POST" key={`delete-${phoneId}`}>
