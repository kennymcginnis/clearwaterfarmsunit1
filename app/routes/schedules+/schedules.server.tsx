...
async function openScheduleAction({
	userId,
	schedule,
	formData,
}: ScheduleActionArgs) {
	const submission = parseWithZod(formData, { schema: ActionFormSchema })
...
async function lockScheduleAction({
	userId,
	schedule,
	formData,
}: ScheduleActionArgs) {
...
async function closeScheduleAction({
	userId,
	schedule,
	formData,
}: ScheduleActionArgs) {
...
async function closedEmailsAction({ schedule, formData }: ScheduleActionArgs) {
	...
	userSchedules
		...
		.forEach(
			({ ditch, hours, start, stop, user: { emailSubject, primaryEmail } }) => {
				if (primaryEmail) {
					const schedule = {
						ditch,
					...
					if (emails[primaryEmail])
						emails[primaryEmail].schedules.push(schedule)
					else emails[primaryEmail] = { emailSubject, schedules: [schedule] }
				}
			},
		)
	const batchEmails = Object.entries(emails).map(
		([primaryEmail, { emailSubject, schedules }]) => ({
